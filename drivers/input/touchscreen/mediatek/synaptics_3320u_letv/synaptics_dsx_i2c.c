...
#endif
#ifdef PROJECT_X50X
#define SENSOR_MAX_X 1079
#define SENSOR_MAX_Y 1919
#else
...
unsigned char g_log_enable = 0;
static unsigned char g_TouchPoint = 0;
...
#endif
static unsigned int shipping_customer_configuration_id_truly = 0x3005704;
...
static u64 gpwDMABuf_pa = NULL;
static u8 *gprDMABuf_va = NULL;
static u64 gprDMABuf_pa = NULL;
...
static int synaptics_rmi4_suspend(struct early_suspend *h);

static int synaptics_rmi4_resume(struct early_suspend *h);
...
extern ssize_t (*synaptics_short_circuit_test_show)(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t (*synaptics_do_afe_calibration_show)(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t (*synaptics_calibration_data_freq0_show)(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t (*synaptics_calibration_data_freq1_show)(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t (*synaptics_calibration_data_freq2_show)(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t (*synaptics_calibration_data_test_show)(struct device *dev, struct device_attribute *attr, char *buf);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	gpwDMABuf_va = (u8 *)dma_alloc_coherent(&(rmi4_data->input_dev->dev), 1024 * 5, &gpwDMABuf_pa, GFP_KERNEL);
    if(!gpwDMABuf_va){
    	printk("[Error] Allocate DMA I2C Buffer failed!\n");
		retval = -1;
		goto exit;
		...
		unsigned int intr_count)
{
	...
	/* Maximum x and y */
rmi4_data->sensor_max_x = SENSOR_MAX_X;//control_6_9.sensor_max_x_pos_7_0 |
...
* respectively.
*/
void synaptics_rmi4_new_function_l(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
EXPORT_SYMBOL(synaptics_rmi4_new_function_l);

static ssize_t reset_gpio_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int retval;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
    retval = synaptics_rmi4_reset_device_gpio(rmi4_data);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
		...
		struct device_attribute *attr, char *buf)
{
    ...
    if(!strcmp(rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||!strcmp(rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01")){
        if(rmi4_data->config_id == shipping_customer_configuration_id_truly)
            return sprintf(buf, "Pass\n");
        else
            return sprintf(buf,"TP FW check FAIL,\nshipping FW = 0x%x, \nlocal FW = 0x%x\n", 
                    shipping_customer_configuration_id_truly, rmi4_data->config_id);
    }
    return 0;
}

static ssize_t synaptics_dynamic_log_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
...
static ssize_t synaptics_dynamic_log_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"%d\n", g_log_enable);
}
static ssize_t synaptics_touch_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int retval;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	return sprintf(buf,"Pass\n");
}
static ssize_t synaptics_touch_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
    int retval;
    int cmd, arg;
	unsigned char force_cal = 0x02;
    if(sscanf(buf,"%d%d", &cmd, &arg) < 0)
        return -EINVAL;

    switch(cmd)
    {
        case 0:                         //calibration for auo
            printk("@@@ %s : do force calibration\n",__func__);
            /* Do force cal */ 
            retval = synaptics_rmi4_i2c_write(rmi4_data,
                    0x015c, &force_cal, 1);
            if (retval < 0) {
                dev_err(dev, "%s: Failed to do force cal\n",
                        __func__);
                return;
            }	
...
static ssize_t synaptics_rmi4_gesture_reg_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	unsigned char offset;
	...
	return sprintf(buf,"reg: 0x%x 0ffset: %d status: %d\n", fhandler->full_addr.ctrl_base, offset, reporting_control[2]);
}


static DEVICE_ATTR(reset_gpio, S_IRUGO | S_IWUGO, synaptics_rmi4_show_error, reset_gpio_store);
...
static DEVICE_ATTR(dynamic_log, S_IRUGO | S_IWUGO, synaptics_dynamic_log_show, synaptics_dynamic_log_store);
static DEVICE_ATTR(touch_debug, S_IRUGO | S_IWUGO, synaptics_touch_debug_show, synaptics_touch_debug_store);
static DEVICE_ATTR(stay_wake, S_IRUGO, synaptics_rmi4_f01_stay_wake_show, synaptics_rmi4_f01_stay_wake_store);
static DEVICE_ATTR(product_id, S_IRUGO, synaptics_rmi4_f01_product_id_show, synaptics_rmi4_store_error);
static DEVICE_ATTR(mulTouchtest, S_IRUGO | S_IWUGO, mulTouchtest_show, synaptics_rmi4_store_error);
static DEVICE_ATTR(F01_RMI_CTRL00, S_IRUGO, synaptics_rmi4_F01_RMI_CTRL00_show, synaptics_rmi4_store_error);
static DEVICE_ATTR(wake_gesture_enable, S_IRUGO | S_IWUGO, synaptics_rmi4_wake_gesture_show, synaptics_rmi4_wake_gesture_store);
static DEVICE_ATTR(gesture_reg_status, S_IRUGO | S_IWUGO, synaptics_rmi4_gesture_reg_status_show, synaptics_rmi4_store_error);
...
static int synaptics_open_circuit_test_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_open_circuit_test_show(g_dev, NULL, ptr);
    *ppos += ptr - page;
    return (ptr - page);
}
static int synaptics_short_circuit_test_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_short_circuit_test_show(g_dev, NULL, ptr);
    *ppos += ptr - page;
    return (ptr - page);
}
...
static int synaptics_raw_cap_data_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_raw_cap_data_show(g_dev, NULL, ptr);
...
static int synaptics_do_calibration_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_do_afe_calibration_show(g_dev, NULL, ptr);
...
static int synaptics_calibration_data_freq0_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_calibration_data_freq0_show(g_dev, NULL, ptr);
...
static int synaptics_calibration_data_freq1_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_calibration_data_freq1_show(g_dev, NULL, ptr);
...
static int synaptics_calibration_data_freq2_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_calibration_data_freq2_show(g_dev, NULL, ptr);
...
static int synaptics_calibration_data_test_proc(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
    ...
    ptr += synaptics_calibration_data_test_show(g_dev, NULL, ptr);
...
static struct attribute *synaptics_rmi4_attrs[] = {
	...
	attrify(tp_firmware_version_check),
	attrify(dynamic_log),
	attrify(touch_debug),
	attrify(stay_wake),
	attrify(product_id),
	attrify(mulTouchtest),
	attrify(F01_RMI_CTRL00),
	attrify(wake_gesture_enable),
	attrify(gesture_reg_status),
	NULL,
};
...
#endif
{
	...
	rmi4_data->reset_device_gpio = synaptics_rmi4_reset_device_gpio;
...
static int synaptics_rmi4_suspend(struct early_suspend *h)
{
   ...
   // rmi4_tp_wg.wg_enable = rmi4_data->enable_wakeup_gesture;
	...
	//	synaptics_rmi4_wakeup_gesture(rmi4_data, true);
       ...
       // rmi4_tp_wg.tp_wg_func = synaptics_rmi4_wakeup_gesture;
...
{
	if(!strcmp(mtkfb_find_lcm_driver(),"r69006_dsi_cmd_37_sgm3804") )
	{
